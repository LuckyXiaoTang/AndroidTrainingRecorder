...
class EGLCore {
    ...
    fun init(eglContext: EGLContext?, flag: Int) {
        // 1.创建EGLDisplay
        var shareContext = eglContext ?: EGL14.EGL_NO_CONTEXT

        mEGLDisplay = EGL14.eglGetDisplay(EGL14.EGL_DEFAULT_DISPLAY)
        if (mEGLDisplay == EGL14.EGL_NO_DISPLAY) throw Throwable("Unable to get EGL14 display")
        ...
        if (mEGLContext == EGL14.EGL_NO_CONTEXT) {
            val config =
                getConfig(flag, 2) ?: throw Throwable("Unable to find a suitable EGLConfig")
            val attrList = intArrayOf(EGL14.EGL_CONTEXT_CLIENT_VERSION, 2, EGL14.EGL_NONE)
            val context = EGL14.eglCreateContext(mEGLDisplay, config, shareContext, attrList, 0)
            mEGLConfig = config
            mEGLContext = context
        }
    }
...
//            EGL14.EGL_STENCIL_SIZE,8,
    ...
    fun createWindowSurface(surface: Any): EGLSurface {
        ...
        return EGL14.eglCreateWindowSurface(mEGLDisplay, mEGLConfig, surface, attr, 0)
    ...
    fun makeCurrent(eglSurface: EGLSurface) {
        if (mEGLDisplay == EGL14.EGL_NO_DISPLAY) throw Throwable("EGLDisplay is null, call init first")
     ...
     * @param nsecs 时间戳(纳秒)
     */
    fun setPresentationTimestamp(eglSurface: EGLSurface, time: Long) {
